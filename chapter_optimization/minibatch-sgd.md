...
각 반복에서 그라디언트 디센트는 전체  학습 데이터 셋을 사용하여 그라데이션을 계산하므로 배치 그라디언트 디센트라고도 합니다. SGD (Stochastic gradient descent)는 각 반복에서 임의로 하나의 예제 만 선택하여 그라디언트를 계산합니다. 이전 장에서 처럼 각 반복마다 무작위 균일 샘플링(random uniform sampling)을 수행하여 미니 배치(mini-batch)를 만들고 그 미니 배치를 사용하여 그라디언트를 계산할 수 있습니다.  지금부터, 미니 배치 스토캐스틱 그라디언트 디센트(mini-batch stochastic gradient descent)에 대해 논의 할 것입니다.
...
목적 함수 $ f(\boldsymbol{x}) : \mathbb {R} ^ d \rightarrow \mathbb {R} $을 설정하십시오. 반복 시작 전의 시간 단계는 0으로 설정됩니다. 이 시간 단계의 독립 변수는 $ \boldsymbol {x} _0 \in \mathbb {R} ^ d $이며 대개 무작위 초기화로 얻어집니다. 이후의 각 단계 $ t> 0 $에서 미니 배치 SGD는 무작위 균일 샘플링(random uniform sampling)을 사용하여 학습 데이터 셋의 예제 색인으로 만들어진 미니 배치 $ \mathcal {B} _t $를 얻습니다. 대체하는 샘플링(sampling with replacement)을 사용하거나 샘플링을 사용하여 미니 배치 예제를 얻을 수 있습니다. 이전 방법은 동일한 미니 배치에서 중복 된 예제를 허용하고, 후자는 더 일반적으로 사용되지 않습니다. 두 가지 방법 중 하나를 사용할 수 있습니다.
